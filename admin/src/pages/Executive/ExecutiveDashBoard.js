...
function ExecutiveDashBoard() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const userOrders = data.filter((order) => order.stockistEmail === email);
        ...
        const totalProducts = userOrders.reduce(
          (sum, order) =>
            sum +
            order.products.reduce((productSum, product) => productSum + product.quantity, 0),
          0
        );
  ...
  return (
    <div className=" bg-[#DBEAFE] ml-80 py-4 ">
      <Navbar />
      ...
      <div className="flex flex-col lg:flex-row items-center justify-between gap-6 lg:gap-10 bg-gradient-to-r from-blue-500 via-teal-500 to-green-500 rounded-xl mx-4 lg:mx-10 my-6 p-6 lg:h-44">
        <Avatar alt="User Avatar" src={Img} className="w-16 h-16 lg:w-20 lg:h-20" />
        {/* <p className="font-medium text-xl text-black-400">{new Date().toLocaleDateString()}</p> */}
        <div className="text-center">
          <p className="text-lg lg:text-2xl font-bold text-white">{localStorage.getItem("email")}</p>
      ...
      <div className="container mx-auto bg-[#DBEAFE]  p-4 rounded-lg shadow-lg lg:px-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="card bg-white p-6 rounded-lg shadow-lg gap-6">
            ...
            <p className="font-bold text-2xl text-green-500">₹ {monthlySales}</p>
          </div>
          
          <div className="card bg-white p-6 rounded-lg shadow-lg gap-6">
            <p className="font-medium text-lg text-gray-600">Monthly Orders</p>
            <p className="font-bold text-2xl text-blue-500">{monthlyOrderCount}</p>
          </div>
          <div className="card bg-white p-6 rounded-lg shadow-lg gap-6">
          ...
          <div className="card bg-white p-6 rounded-lg shadow-lg gap-6">
            ...
            <p className="font-bold text-2xl text-blue-500">{todayOrderCount}</p>
          </div>
        </div>
