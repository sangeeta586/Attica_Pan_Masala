...
export const AreaSellManagerRegister = ({ onClose,selectAreaSellManager }) => {
    const [formData, setFormData] = useState({
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          ...
          const response = selectAreaSellManager
            ? await axios.put(`${BASE_URL}/api/areaSealManger/${selectAreaSellManager._id}`, dataToSend)
            : await axios.post(`${BASE_URL}/api/areaSealManger/register`, dataToSend);
    
          toast.success(selectAreaSellManager ? "User updated successfully" : "User registered successfully");
      ...
      useEffect(() => {
        if (selectAreaSellManager) {
          setFormData({
            username: selectAreaSellManager.username,
            email: selectAreaSellManager.email,
            phoneNo: selectAreaSellManager.phoneNo,
            address: selectAreaSellManager.address,
            city: selectAreaSellManager.city,
            pinCode: selectAreaSellManager.pinCode,
            state: selectAreaSellManager.state,
            stockist: selectAreaSellManager.stockist || null,
          });
        }
      }, [selectAreaSellManager]);
      ...
      return (
        <div className="flex justify-center items-center h-full">
          ...
          <Typography variant="h4" className="text-[#1e40af]">
              {selectAreaSellManager ? "Update Delivery Boy" : "Registration"}
            </Typography>
            ...
            <div className="overflow-y-auto max-h-[80vh] p-4">
              <form className="mt-8 mb-2 max-w-screen-lg sm:w-96" onSubmit={handleSubmit}>
                ...
                {selectAreaSellManager ? "Update" : "Sign up"}
              </Button>
              </form>
            </div>
